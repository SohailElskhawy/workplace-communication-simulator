...
September 5 design exploration: created `docs/design-preview/index.html` and `docs/design-preview/UX_REVIEW.md` as a standalone, research-informed UI/UX proposal for review before implementation. Includes Home, Conversation, Results, font comparisons, and simulated voice/recovery interactions. The audit highlights the current 1.8-second silence auto-stop, moving recording controls, broad keyboard handlers, and draft/permission recovery risks. This does not replace the approved design or change application behavior; proposed interaction changes remain pending design review.
...
September 4 UI/UX system redesign: audited the complete authenticated frontend, route hierarchy, reusable components, visual tokens, interaction states, voice flows, results disclosure, responsive patterns, and RTL debt before implementation. The audit and functional-parity gate are recorded in `docs/UX_AUDIT_AND_REDESIGN_PLAN.md`; `docs/DESIGN.md` now defines the Calm Coaching system. The authenticated shell now exposes Home, Practice, Interview prep, and Progress; the homepage gives Recommended Practice, Browse Scenarios, and Real Interview Prep equal visual weight; the scenario library adds search and consistent in-grid recommendation; custom interview copy is value-led; result detail uses progressive disclosure; and conversation/transcript layout uses role/logical direction primitives. Push-to-talk no longer disables during counterpart TTS, starting the microphone interrupts active playback, and hold-Space recording is available outside editable fields while click and text fallback remain intact. No API, database, scoring, authorization, privacy, AI prompt, attempt lifecycle, entitlement, or persistence behavior changed. Verified with 496 passing tests across 72 test files, strict web typecheck, clean production build, and ESLint with no errors.

September 2 addition — Custom Scenario Generation, Deletion & Prominent Placement: authenticated users across all tiers (including Free demo tier) can generate personalized interview simulations by uploading their CV (PDF) and providing a job description. The backend parses the PDF in-memory (zero persistence of CV files), validates schema constraints, and invokes OpenRouter with strict prompt instructions and JSON fence stripping to generate an immutable custom scenario saved with `userId` and `isCustom: true`. To maintain clean catalogs, users can delete their custom scenarios (`DELETE /api/v1/scenarios/:scenarioKey`), which safely soft-deletes active records (`isActive: false`) to preserve relational integrity with past simulation attempts while removing the scenario from library listings. Curated/system scenarios remain immutable and protected against deletion (403 Forbidden). Delete controls and accessible confirmation dialogs are integrated into both the Scenario Library (`ScenarioLibraryView`) and Scenario Detail (`ScenarioDetailPage`) pages. Furthermore, the entry point for creating custom interviews was upgraded to an eye-catching, high-energy Neo-Brutalist Spotlight widget in the Scenario Library hero header, a dedicated "+ Custom Interview" filter action pill, a dedicated interactive "Build Custom Interview" card within the scenario catalog grid, an empty state trigger for custom filter views, and a direct action in the Dashboard practice section. Verified with 495 tests across 72 test suites, clean typecheck, ESLint, and Next.js production build.
...
26. September 2 responsive landing-page pricing section: added a dedicated, responsive Neo-Brutalist pricing section (`PricingSection`) and centralized plan config (`PRICING_PLANS`, `formatWhatsAppUrl`) in `apps/web/src/lib/pricing-config.ts`. Displays Free ($0 forever, 3 simulations/week), Plus ($15/mo, 15 simulations/week, Most Popular), and Pro ($29/mo, unlimited simulations, Power User). Free tier links to signup/app modal, while paid CTAs open WhatsApp with prefilled plan-specific upgrade messages directed to `+905528509969`. Updated navigation links (desktop nav, mobile menu, and footer) to include `#pricing`. No Stripe, checkout, or backend billing was introduced. Verified with 468 tests across 70 test files passing, strict typecheck, clean lint, Prettier verification, and production build.

27. September 2 custom interview scenario creation & frontend workflow: implemented owner-scoped custom interview scenario generation for authenticated PLUS and PRO learners. Built the end-to-end frontend wizard flow (`Create Interview → CV PDF → job description → difficulty → generate → review summary → start`) via `CustomInterviewWizard`, accessible both via modal (`CreateCustomScenarioDialog`) and dedicated route (`/app/scenarios/custom`). Learners upload a candidate CV (PDF up to 5MB, validated in memory) and paste their target job description (50–20,000 characters). The CV is parsed strictly in-memory using `unpdf` with zero document or buffer persistence to disk or database. AI prompt (`custom-scenario-v1`) enforces strict factual grounding against CV achievements and JD requirements with zero hallucinations. Output is runtime-validated using `ScenarioDefinitionSchema` and persisted to PostgreSQL as an active scenario with `category: "CUSTOM"` and `userId: user.id`. Free tier users receive a 403 `PLAN_UPGRADE_REQUIRED` error with a concierge upgrade prompt. After generation, the learner reviews the scenario summary, context (candidate role, interviewer role, objective, stakes), selects target difficulty and interaction mode, and immediately begins simulation practice. Attempt, turn, roleplay, finishing, and structured evaluation seamlessly execute across custom scenarios with strict owner isolation (non-owners receive 404). Verified with migration `20260902010000_add_custom_scenarios`, 487 tests across 72 test files passing, strict typecheck, clean lint, Prettier verification, and successful Next.js production build.
