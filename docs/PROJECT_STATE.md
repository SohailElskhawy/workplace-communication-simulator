...
19. Realtime voice backend bootstrap (ElevenLabs) — backend only, no frontend work yet. Adds server-only `ELEVENLABS_API_KEY`, `ELEVENLABS_AGENT_ID`, and `ELEVENLABS_TOOL_SECRET` settings (optional; the feature is enabled only when all three are configured). New authenticated `POST /api/v1/attempts/:attemptId/realtime-session` validates ownership and `ACTIVE` state, issues a short-lived ElevenLabs WebRTC conversation token server-side, and returns only public scenario data (resolved `openingMessage`, scenario key/version/title, difficulty, attempt expiry) plus a short-lived HMAC-signed context token bound to the attempt and user. New ElevenLabs-only `POST /api/v1/realtime/scenario-context` is protected by the tool secret plus the signed context token, loads the attempt server-side, resolves its stored `variationId`, difficulty, and scenario version (never selecting a new variation), and returns the hidden realtime roleplay context — the exact `roleplay-v2` system prompt content shared with text roleplay via a refactored `buildRoleplaySystemPrompt`. The realtime session endpoint is rate-limited as an expensive AI request; provider failures map to the existing `AI_TIMEOUT`/`AI_PROVIDER_ERROR` codes. No DB schema changes, no transcript persistence or webhooks, and text/STT/TTS/evaluation flows are unchanged.
