...
27. September 2 custom interview scenario creation: implemented owner-scoped custom interview scenario generation for authenticated PLUS and PRO learners. Learners upload a candidate CV (PDF up to 5MB) and paste their target job description (50–20,000 characters). The CV is parsed strictly in-memory using `unpdf` with zero document or buffer persistence to disk or database. AI prompt (`custom-scenario-v1`) enforces strict factual grounding against CV achievements and JD requirements with zero hallucinations. Output is runtime-validated using `ScenarioDefinitionSchema` and persisted to PostgreSQL as an active scenario with `category: "CUSTOM"` and `userId: user.id`. Free tier users receive a 403 `PLAN_UPGRADE_REQUIRED` error with a concierge upgrade prompt. Attempt, turn, roleplay, finishing, and structured evaluation seamlessly execute across custom scenarios with strict owner isolation (non-owners receive 404). Frontend includes an interactive modal (`CreateCustomScenarioDialog`), drag-and-drop PDF upload, JD input, custom badges, and category filters. Verified with migration `20260902010000_add_custom_scenarios`, 485 tests across 72 test files passing, strict typecheck, clean lint, and Prettier verification.
